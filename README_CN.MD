...
使用nodelet节点打开 gemini2 深度相机
```bash
roslaunch orbbec_camera gemini2_nodelet.launch
```
注意：
- `enable_imu_sync` : 使用 ApproximateTime 时间同步机制同步 `accel/sample` 与 `gyro/sample` 的数据
- `accel_rate` 和 `gyro_rate` 最好保持一致
- 使用 rostopic 显示 `accel/sample` 与 `gyro/sample` 的数据/频率/类型等后，需重启节点才能继续使用 `imu/sample`
